...
static void status_icon_activate (GtkStatusIcon * icon,GdkEventScroll *event, gpointer data);
...
app_indicator_class_init (AppIndicatorClass *klass)
{
	...
	*/
	 ...
	 * Signaled when the #AppIndicator receives a scroll event.
	 */
	signals[ACT] = g_signal_new (APP_INDICATOR_SIGNAL_ACT,
	                                  G_TYPE_FROM_CLASS(klass),
	                                  G_SIGNAL_RUN_LAST,
	                                  G_STRUCT_OFFSET (AppIndicatorClass, act),
	                                  NULL, NULL,
	                                  _application_service_marshal_VOID__INT_UINT,
	                                  G_TYPE_NONE, 0);
                 ...
                 GDBusMethodInvocation * invocation, gpointer user_data)
{
	...
	if (g_strcmp0(method, "Scroll") == 0) {
		    ...
		    gtk_widget_get_sensitive (menuitem))
		{
			...
			//g_signal_emit(app, signals[ACT], 0);
		}
	} else if (g_strcmp0(method, "ShowEntry") == 0){
        g_print("\n\n");
        g_signal_emit(app, signals[ACT], 0);
...
fallback (AppIndicator * self)
{
	...
	//
	g_signal_connect(G_OBJECT(icon), "act", G_CALLBACK(status_icon_activate), self);
	//
...
/* Handles the activate action by the status icon by showing
   the menu in a popup. */
static void
status_icon_activate (GtkStatusIcon * icon, GdkEventScroll *event, gpointer data)
{
    return;
